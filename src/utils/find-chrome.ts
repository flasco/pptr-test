...
function sort(installations: any, priorities: any) {
  ...
  return (
    installations
      ...
      .map((pair: any) => pair.path)
  );
}

function canAccess(file: any) {
...
function findChromeExecutables(folder: any) {
  ...
  if (canAccess(folder)) {
    ...
    execPaths = execPaths
      .toString()
      .split(newLineRegex)
      .map((execPath: any) => execPath.replace(argumentsRegex, '$1'));

    execPaths.forEach(
      (execPath: any) => canAccess(execPath) && installations.push(execPath)
    );
  }
 ...
 */
async function downloadChromium(options: any, targetRevision?: any) {
...
export default async function findChrome(options: any = {}) {
  if (options.executablePath)
    return { executablePath: options.executablePath, type: 'user' };

  const config: Set<any> = new Set(options.channel || ['stable']);
  let executablePath;
